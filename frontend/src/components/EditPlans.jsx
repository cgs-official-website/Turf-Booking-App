        ...
        <div className="ep-mobile-plan-tabs">
          {plans.map((p) => (
            ...
            >
              ...
              <div className="ep-mobile-plan-tab__name">{p.duration}</div>
...
// //       console.log('📥 Loading plans from:', 'http://localhost:5000/subscriptions/plans?includeInactive=true');
      
// //       const response = await axios.get('http://localhost:5000/subscriptions/plans', {
...
// //         planData = response.data.data.plans;
...
// //       console.log('📋 Plans from backend:', planData);
      
// //       const editPlans = planData.map(fromBackendToEdit);
// //       console.log('✅ Converted plans:', editPlans);
...
// //       if (planToDelete._id && !planToDelete._id.toString().startsWith('temp_')) {
// //         const token = getAuthToken();
// //         await axios.delete(`http://localhost:5000/subscriptions/plans/${planToDelete._id}`, {
// //           headers: {
// //             'Authorization': `Bearer ${token}`,
// //             'Content-Type': 'application/json'
// //           }
...
// //   // ✅ MAIN SAVE FUNCTION - Sends to backend
// //   const handleSaveChanges = async () => {
...
// //       for (let i = 0; i < updatedPlans.length; i++) {
// //         const plan = updatedPlans[i];
// //         console.log(`💾 Saving plan ${i + 1}/${updatedPlans.length}:`, plan.name);
...
// //           console.log(`🔄 Updating plan: ${plan._id}`);
// //           response = await axios.put(
// //             `http://localhost:5000/subscriptions/plans/${plan._id}`,
// //             payload,
// //             {
// //               headers: {
// //                 'Authorization': `Bearer ${token}`,
...
// //             'http://localhost:5000/subscriptions/plans',
// //             payload,
// //             {
// //               headers: {
// //                 'Authorization': `Bearer ${token}`,
...
// //           let newId = null;
// //           if (response.data?.data?.plan?._id) {
// //             newId = response.data.data.plan._id;
// //           } else if (response.data?.plan?._id) {
// //             newId = response.data.plan._id;
// //           } else if (response.data?.data?._id) {
// //             newId = response.data.data._id;
// //           } else if (response.data?._id) {
// //             newId = response.data._id;
// //           }
          
// //           if (newId) {
// //             updatedPlans[i]._id = newId;
// //             updatedPlans[i].id = newId;
// //             console.log(`✅ New plan ID from backend: ${newId}`);
// //           }
// //         }
// //         savedCount++;
// //       }
...
// //       // Reload from backend
// //       console.log('🔄 Reloading plans from backend...');
// //       await loadPlans();
...
//         // Cover every possible shape the backend might return
//         const newId =
//           response?.data?.data?.plan?._id ||
...
//     // Reload from backend so the list reflects exactly what was persisted
//     await loadPlans();

//     // Notify parent (Subscriptions.jsx) so its plan list refreshes too
//     if (onSave) {
//       await onSave();
//     }

//     // ✅ Do NOT call onBack() — let the admin confirm save worked
//     //    before pressing Back themselves.
...
// // Convert frontend (EditPlans) plan to backend payload.
// // Field names here must match the Plan model / Joi validators exactly:
// // featureList (array of strings) and isMostPopular (boolean) are the
// // two fields that previously had no matching schema field on the
// // backend, which is why saved changes never persisted correctly.
// const fromEditToBackend = (editPlan) => {
//   return {
//     name: editPlan.name,
//     description: editPlan.tagline || "",
...
//   // ── Load plans from backend (includes inactive, since this is the admin view) ──
//   const loadPlans = async () => {
//     setIsLoading(true);
//     setError(null);
...
//       // ApiResponse shape: { success, statusCode, message, data: { count, plans } }
//       let planData = [];
//       if (response?.data?.plans) {
//         planData = response.data.plans;
//       } else if (Array.isArray(response?.data)) {
...
//           return { ...p, isMostPopular: willBePopular };
//         }
//         // Only one plan can be "Most Popular" — uncheck the rest locally too,
//         // matching what the backend will enforce on save.
//         if (willBePopular) {
//           return { ...p, isMostPopular: false };
//         }
//         return p;
...
//   // ── Save all plans to backend, then refresh from backend so the UI
//   //    always reflects what is actually persisted in MongoDB ──
//   const handleSaveChanges = async () => {
//     if (!hasChanges) {
//       toast('No changes to save');
//       return;
...
//       // Reload straight from backend so the list/preview reflect exactly
//       // what was persisted (including server-side "only one most popular"
//       // enforcement).
//       await loadPlans();
