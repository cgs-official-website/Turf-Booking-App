...
const FEATURE_MAX_LEN = 50;
const PLAN_NAME_MAX_LEN = 30;
const FEATURES_MAX_COUNT = 8;
...
export default function EditPlans({ onBack, onSave }) {
  ...
  const confirmAddFeature = () => {
    ...
    if (selected.features.length >= FEATURES_MAX_COUNT) {
      toast.error(`You can add up to ${FEATURES_MAX_COUNT} features only`);
      return;
    }
    updateSelected("features", [...selected.features, newFeatureText.trim().slice(0, FEATURE_MAX_LEN)]);
    setNewFeatureText("");
    setAddingFeature(false);
  };

  const startEditingFeature = (idx, currentText) => {
    setEditingFeatureIndex(idx);
    setEditingFeatureText(currentText);
  };

  const confirmEditFeature = () => {
    if (!selected || editingFeatureIndex === null) return;
    const trimmed = editingFeatureText.trim().slice(0, FEATURE_MAX_LEN);
    if (trimmed) {
      const updatedFeatures = selected.features.map((f, i) =>
        i === editingFeatureIndex ? trimmed : f
      );
      updateSelected("features", updatedFeatures);
    }
              ...
              <div className="ep-field">
                ...
                <div className="ep-textarea-wrapper">
                  <input
                    ...
                    className="ep-input"
                    value={selected.name}
                    maxLength={PLAN_NAME_MAX_LEN}
                    onChange={(e) => updateSelected("name", e.target.value.slice(0, PLAN_NAME_MAX_LEN))}
                    disabled={isSaving}
                  />
                  <span className="ep-char-count">{selected.name.length}/{PLAN_NAME_MAX_LEN}</span>
                </div>
              </div>
              ...
              <div className="ep-features-list">
                {selected.features.map((feature, index) => (
                  <div key={index} className="ep-feature-item">
                    ...
                    {editingFeatureIndex === index ? (
                      <div className="ep-feature-edit-wrapper">
                        <input
                          type="text"
                          className="ep-feature-edit-input"
                          value={editingFeatureText}
                          maxLength={FEATURE_MAX_LEN}
                          autoFocus
                          onChange={(e) => setEditingFeatureText(e.target.value)}
                          onKeyDown={(e) => {
                            if (e.key === "Enter") confirmEditFeature();
                            if (e.key === "Escape") cancelEditFeature();
                          }}
                          onBlur={confirmEditFeature}
                          disabled={isSaving}
                        />
                        <span className="ep-feature-edit-count">
                          {editingFeatureText.length}/{FEATURE_MAX_LEN}
                        </span>
                      </div>
                    ) : (
                      <span
                        className="ep-feature-text"
                        title={feature}
                        onClick={() => !isSaving && startEditingFeature(index, feature)}
                      >
                        {feature}
                      </span>
                    )}
                ...
                {addingFeature ? (
                  <div className="ep-add-feature-form">
                    <div className="ep-feature-edit-wrapper" style={{ flex: 1, minWidth: 150 }}>
                      <input
                        type="text"
                        className="ep-input"
                      ...
                      />
                      <span className="ep-feature-edit-count">
                        {newFeatureText.length}/{FEATURE_MAX_LEN}
                      </span>
                    </div>
                ...
                ) : selected.features.length >= FEATURES_MAX_COUNT ? (
                  <p className="ep-feature-limit-note">
                    Maximum {FEATURES_MAX_COUNT} features reached
                  </p>
                ) : (
                  <button
                  ...
                  >
                    <FiPlus size={16} />
                    Add New Feature ({selected.features.length}/{FEATURES_MAX_COUNT})
                  </button>
                )}
              </div>
