...
// ── Extract the new _id from any common API response shape ──
const extractNewId = (response) =>
  response?.data?._id ||
  response?.data?.plan?._id ||
...
export default function EditPlans({ onBack, onSave }) {
  ...
  const toggleMostPopular = () => {
    ...
    setPlans((prev) =>
      prev.map((p) => {
        ...
        if (willBePopular)        return { ...p, isMostPopular: false };
        return p;
      })
    );
    setHasChanges(true);
  ...
  const addNewPlan = () => {
    ...
    const newPlan = {
      id: tempId,
      _id: null,           // null signals "needs CREATE, not UPDATE"
      name: "New Plan",
  ...
  const handleDeletePlan = async () => {
    if (plans.length === 1) { toast.error('Cannot delete the last plan'); return; }
    const planToDelete = plans.find((p) => p.id === selectedId);
    try {
      if (planToDelete._id && !planToDelete.id.startsWith('temp_')) {
  ...
  const handleDurationChange = (val) => {
    ...
    setPlans((prev) =>
      prev.map((p) => {
        if (p.id !== selectedId) return p;
        setHasChanges(true);
        return { ...p, duration: val, billingLabel: durationToBilling(val) };
      })
    );
  };

  // ── Save all plans to backend ──
  // NEW plans  → createPlan  (id starts with "temp_" OR _id is null)
  // EXISTING   → updatePlan
  // After all saves: call onSave() (reloads parent) then onBack() — no timeout.
  // In EditPlans.jsx, update the handleSaveChanges function:
...
const handleSaveChanges = async () => {
  ...
  try {
    ...
    for (let i = 0; i < updatedPlans.length; i++) {
      ...
      // Detect new plan: _id is null OR id still starts with 'temp_'
      const isNew = !plan._id || plan.id.toString().startsWith('temp_');

      if (isNew) {
        // ── CREATE ──
        const response = await subscriptionApi.createPlan(payload);
        const newId = extractNewId(response);
        console.log('[EditPlans] createPlan response:', response, '→ newId:', newId);
        // Update the local plan entry so subsequent saves (if any) use the real _id
        if (newId) {
    ...
    // Update the plans state with the new IDs
    setPlans(updatedPlans);
    setHasChanges(false);
    
    toast.success(`Successfully saved ${savedCount} plan(s)!`);

    // Reload parent Plan Management cards FIRST, then navigate back.
    if (onSave) {
      await onSave(updatedPlans); // Pass the updated plans
    }
    if (onBack) {
      onBack();
    }
    ...
    <div className="ep-container">
      ...
      <div className="ep-mobile-plan-tabs">
        {plans.map((p) => (
          ...
          >
            ...
            <div className="ep-mobile-plan-tab__price">
              ...
              <span className="ep-mobile-plan-tab__per"> / {getPeriodLabel(p.duration)}</span>
            </div>
        ...
        <div className="ep-mobile-plan-tab ep-mobile-plan-tab--add" onClick={addNewPlan}>
          <div className="ep-mobile-plan-tab__add-icon"><FiPlus size={22} /></div>
      ...
      <div className="ep-mobile-actions">
        ...
        <button className="ep-save-btn" onClick={handleSaveChanges} disabled={isSaving || !hasChanges}>
          {isSaving ? <><FiLoader className="ep-spin" size={15} /> Saving...</> : <><FiSave size={15} /> Save Changes</>}
        </button>
      </div>
      ...
      <div className="ep-header">
        <div className="ep-header-left">
          ...
          {hasChanges && <span className="ep-unsaved-badge">● Unsaved changes</span>}
        </div>
        <div className="ep-header-actions">
          ...
          <button className="ep-save-btn" onClick={handleSaveChanges} disabled={isSaving || !hasChanges}>
            {isSaving ? <><FiLoader className="ep-spin" size={16} /> Saving...</> : <><FiSave size={16} /> Save Changes</>}
          </button>
        </div>
      </div>
      ...
      <div className="ep-main">
        ...
        <div className="ep-left">
            ...
            {plans.map((p) => (
              ...
              >
                <div className="ep-plan-card-top">
                  ...
                  {p.isMostPopular && (
                    <span className="ep-most-popular-badge"><FiStar size={10} /> Most Popular</span>
                  )}
                </div>
          ...
          <button className="ep-add-plan" onClick={addNewPlan} disabled={isSaving}>
          ...
          {selected ? (
            <>
              ...
              <div className="ep-field">
                ...
                <input
                  type="text" className="ep-input" value={selected.name}
              ...
              <div className="ep-field">
                ...
                <div className="ep-textarea-wrapper">
                  <textarea
                    className="ep-textarea" value={selected.tagline}
                    onChange={(e) => updateSelected("tagline", e.target.value)}
                    rows={2} maxLength={150} disabled={isSaving}
                  />
...
placeholder="0" min={0}
                ...
                <div className="ep-field">
                  ...
                  <select
                    className="ep-input ep-select" value={selected.duration}
              ...
              <div className="ep-features-list">
                {selected.features.map((feature, index) => (
                  <div key={index} className="ep-feature-item">
                    <FiCheck className="ep-check" size={15} color={selected.isMostPopular ? "#7c3aed" : "#22c55e"} />
                    <span className="ep-feature-text">{feature}</span>
                    <button className="ep-feature-delete" onClick={() => deleteFeature(index)} disabled={isSaving}>
                ...
                {addingFeature ? (
                  <div className="ep-add-feature-form">
                    <input
                      type="text" className="ep-input" placeholder="Enter feature name..."
                    ...
                    <div className="ep-add-feature-actions">
                      <button className="ep-add-feature-confirm" onClick={confirmAddFeature} disabled={isSaving}>Add</button>
                      <button className="ep-add-feature-cancel" onClick={() => { setAddingFeature(false); setNewFeatureText(""); }} disabled={isSaving}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <button className="ep-add-feature-btn" onClick={() => setAddingFeature(true)} disabled={isSaving}>
            ...
            <div className="ep-no-plan-selected">
              ...
              <button className="ep-add-plan" onClick={addNewPlan}><FiPlus size={16} /> Add a plan</button>
            </div>
          )}
        </div>
          ...
          {selected ? (
            <div className={`ep-preview ${selected.isMostPopular ? 'ep-preview-popular' : ''}`}>
              {selected.isMostPopular && (
                <div className="ep-preview-popular-tag"><FiStar size={12} /> Most popular</div>
              )}
              ...
              <div className="ep-preview-price">
                ...
                <span className="ep-preview-amount">{selected.price.toLocaleString("en-IN")}</span>
                <span className="ep-preview-period">/{getPeriodLabel(selected.duration)}</span>
              </div>
              <ul className="ep-preview-features">
                {selected.features.map((feature, index) => (
                  <li key={index}>
                    <FiCheck size={14} color={selected.isMostPopular ? "#7c3aed" : "#22c55e"} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="ep-preview-empty"><p>Select or create a plan to preview</p></div>
          )}
        </div>
...
// } from "react-icons/fi";
// import { toast } from 'react-hot-toast';
// // ✅ NO AdminLayout import — EditPlans is always rendered INSIDE a page
// //    that is already wrapped in AdminLayout (e.g. Subscriptions.jsx).
// //    Importing it here caused the double-navbar bug shown in the screenshot.
// import * as subscriptionApi from '../services/subscription.service';
// import '../assets/styles/EditPlans.css';
...
//   // ── Loading state — NO AdminLayout wrapper ──
//   if (isLoading) {
//     return (
//       <div className="ep-container">
//         <div className="ep-loading">
...
//   // ── Error state — NO AdminLayout wrapper ──
//   if (error && plans.length === 0) {
//     return (
//       <div className="ep-container">
//         <div className="ep-error">
...
//   // ── Main render — NO AdminLayout wrapper ──
//   return (
//     <div className="ep-container">
...
//         {/* Add Plan card */}
//         <div
//           className="ep-mobile-plan-tab ep-mobile-plan-tab--add"
//           onClick={addNewPlan}
//         >
...
//           {isSaving ? (
//             <>
//               <FiLoader className="ep-spin" size={15} />
//               Saving...
//             </>
//           ) : (
//             <>
